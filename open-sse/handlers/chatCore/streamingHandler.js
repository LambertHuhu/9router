 ...
 */
export function handleStreamingResponse({ providerResponse, provider, model, sourceFormat, targetFormat, userAgent, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, streamController, onStreamComplete, contextSize, requestMeta }) {
  ...
  const transformedBody = pipeWithDisconnect(providerResponse, transformStream, streamController, { contextSize, onAbortTerminal });
 ...
 */
export function buildOnStreamComplete({ provider, model, connectionId, apiKey, requestStartTime, body, stream, finalBody, translatedBody, clientRawRequest, requestMeta }) {
  const streamDetailId = `${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
  ...
  const onStreamComplete = (contentObj, usage, ttftAt) => {
    ...
    saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, label: "STREAM USAGE", meta: requestMeta });
  };
